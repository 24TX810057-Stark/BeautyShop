	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (idParam != null && !idParam.isEmpty()) {
			try {
				...
				// Load sản phẩm theo category + sort
				List<Product> products = productDAO.findByCategory(id, sort);
				...
				// id không hợp lệ → load tất cả + sort
				List<Product> allProducts = productDAO.findAll(sort);
				req.setAttribute("products", allProducts);
			}
		} else {
			// Không có id → load tất cả + sort
			List<Product> allProducts = productDAO.findAll(sort);
			req.setAttribute("products", allProducts);
		}
